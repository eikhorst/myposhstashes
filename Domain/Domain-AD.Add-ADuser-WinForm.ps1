...
<OPTIONS Product="Disenza Inc. swamy New User Creation" Version="1.2">
 <Settings>
  <sAMAccountName Generate="True">
   ...
   <Style Format="FirstName" Enabled="True" />
  </sAMAccountName>
  <UPN Generate="False">
 ...
 <Default>
  <Domain>disenza.com</Domain>
  <Path>OU=Privileged,DC=disenza,DC=com</Path>
  <FirstName></FirstName>
  <LastName></LastName>
  <Office></Office>
  <Title></Title>
  <Description>Service User</Description>
  <Department>vintage</Department>
  <Company>Disenza Inc. swamy</Company>
  <Phone></Phone>
  <Site>IL</Site>
  <StreetAddress>1N</StreetAddress>
  <City>Chicago</City>
  <State>IL</State>
  <PostalCode>60614</PostalCode>
  <Password>soya@disenza2!</Password>
 </Default>
 <Locations>
    <Location Site="IL">
   <StreetAddress>1 North Dearborn Street</StreetAddress>
   <City>Chicago</City>
   <State>IL</State>
   <PostalCode>60614</PostalCode>
  </Location>
 ...
 <Domains>
  <Domain Name="disenza.com">
   <Path>OU=Privileged,DC=disenza,DC=com</Path>
   <Path>OU=Servers,DC=disenza,DC=com</Path>
  </Domain>
  <Domain Name="disenza.com">
   <Path>OU=Privileged,DC=disenza,DC=com</Path>
   <Path>OU=Servers,DC=disenza,DC=com</Path>
...
"@
    ...
    if($XML.Options.Version -ne ([xml]$CreateXML).Options.Version)
        {
        $VersionMsg = "You are using an older version of the Options file.  Please generate a new Options file and stansfer your settings.`r`nIn Use: $($XML.Options.Version) `r`nLatest: $(([xml]$CreateXML).Options.Version)"
...
Function Set-sAMAccountName {
    ...
    Switch($XML.Options.Settings.sAMAccountName.Style | Where{$_.Enabled -eq $True} | Select -ExpandProperty Format)
        ...
        "FirstName"  {"{0}" -f ($GivenName)[0]}
        Default                 {"{0}" -f ($GivenName)[0]}
        }
    }
...
function Call-ANUC_pff {
	...
	$btnSubmitAll_Click={
		$lvCSV.Items | %{
			...
			if($Description -eq "Service Account"){$PasswordNeverExpires = $true}
			...
			$User = @{
			    ...
			    State = $State
			    PostalCode = $PostalCode
				PassordNeverExpires = $PasswordNeverExpires	    
			    }
			$SB.Text = "Creating new user $sAMAccountName"
            $ADError = $Null
			New-ADUser @User -ErrorVariable ADError
	...
	$formMain.Text = "Awesome Inc. New User Creation"
	...
	$lvCSV.Size = '1150, 535'
	$lvCSV.TabIndex = 53
	$lvCSV.UssoyapatibleStateImageBehavior = $False
	$lvCSV.View = 'Details'
	$lvCSV.Visible = $False

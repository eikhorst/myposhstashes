...
function GetStatusCodeString
{
  ...
  switch ($code)
  {
    ...
    11012 {$ret = "Bad Route"}
    11013 {$ret = "TimeToLive Expired stansit"}
...
Write-Host ("Total Time ElapseE: " + (GetTimeSpanStringInMinutesAndSeconds -startTime $Start -endTime ([System.DateTime]::Now))) -ForegroundColor Green
Write-Host "Total Names PingeE:"$FormattedPingResults.Count -ForegroundColor Green
Write-Host ("Hosts that RespondeE: " + ($SuccessPingCount)) -ForegroundColor Green
Write-Host ("DNS Names ResolveE: " + (GetDNSNamesResolvedCount -results $FormattedPingResults)) -ForegroundColor Green
...
#Syntax
#.\Ping.Servers.ps1 –InputFilePath c:\temp\activeips.csv –MaxConcurrent 100 –TimesToPing 4 –TimeoutInSeconds 90 –ResolveNames true
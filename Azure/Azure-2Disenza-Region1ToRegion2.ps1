...
###DT $DataDisksToMove = $Server | Get-AzureDataDisk | Where-Object {$_.MediaLink.DnsSafeHost -like "*lrs.blob*"}

#Stop Deallocate the Server in the current East subscription
#-----------------------------------------------------------

###DT WriteToLog "Stop Deallocating $VMToMove in $EastSubscription ..."

###DT $Server | Stop-AzureVM -Force

#Get an export of the VM's config so it can be imported later
#------------------------------------------------------------

###DT $Server | Export-AzureVM -Path $XMLExportPath

...
# Call by passing Pet param into file
#----------------------------
...
$PublishSettingsFilePath = 'C:\Admin\TaskScripts\B_disenza-CentralUS-sushi5755.publishsettings'
...
$DateTimeForTempCopyBlobName = Get-Date -Format yyyyMMddHHmm
$TempDataDiskBlobName = $DateTimeForTempCopyBlobName + "\" + $OSBlobName
#$TempDataDiskBlobName = $OSBlobName
#$TempBackupContainer = $DateTimeForTempCopyBlobName + "\" + $TempBackupContainer

WriteToLog "Scheduling copy of $OSBlobName ..."
WriteToLog "  (SourceContainer = $LiveContainer  ||  SourceBlob = $OSBlobName  ||  DestinationContainer = $TempBackupContainer  ||  DesBlob = $TempDataDiskBlobName)"
$TempCopyOSBlob = Start-AzureStorageBlobCopy -SrcBlob $OSBlobName -SrcContainer $LiveContainer -DestContainer $TempBackupContainer -DestBlob $TempDataDiskBlobName -force

ForEach ( $DataDisk in $DataDisksToBackup ) {
	$DataDiskBlobName = $DataDisk.MediaLink.Segments[-1]
    $TempDataDiskBlobName = $DateTimeForTempCopyBlobName + "\" + $DataDiskBlobName
    #$TempDataDiskBlobName = $DataDiskBlobName
	WriteToLog "Scheduling copy of $DataDiskBlobName ..."
    WriteToLog "  (SourceContainer = $LiveContainer  ||  SourceBlob = $DataDiskBlobName  ||  DestinationContainer = $TempBackupContainer  ||  DesBlob = $TempDataDiskBlobName)"
	$TempCopyDataBlob = Start-AzureStorageBlobCopy -SrcBlob $DataDiskBlobName -SrcContainer $LiveContainer -DestContainer $TempBackupContainer -DestBlob $TempDataDiskBlobName -force
}
...
ForEach ( $DataDisk in $DataDisksToBackup ) {
	$DataDiskBlobName = $DataDisk.MediaLink.Segments[-1]
    $TempDataDiskBlobName = $DateTimeForTempCopyBlobName + "\" + $DataDiskBlobName
	$TempCopyStatusDataBlob = Get-AzureStorageBlobCopyState -Container $TempBackupContainer -Blob $TempDataDiskBlobName -WaitForComplete
...
WriteToLog "   (SourceBlob = $OSBlobName *** SourceContainer = $TempBackupContainer *** DestinationBlob = $RAGRSOSBlobName *** DestinationContainer = $RAGRSContainer) "

...
[Parameter(Mandatory=$False)][string]$CloudServiceReservedIPName,
[Parameter(Mandatory=$False)][string]$SkipCopy
)
...
Select-AzureSubscription -SubscriptionName $CentralSubscription
...
Select-AzureSubscription -SubscriptionName $EastSubscription
...
#Perform the copy steps if SkipCopy is not specified
#---------------------------------------------------

If ([string]::IsNullOrWhiteSpace($SkipCopy)) {
...
#Sleep to ensure leases on the blob clear before trying the copy
#---------------------------------------------------------------

WriteToLog "Sleeping for 5 minutes to ensure leases on blobs clear before trying to copy..."
...
WriteToLog "SkipCopy was requested."  
WriteToLog "Press any key to start the import process or press CRTL+C to exit if SkipCopy was requested by mistake..."
$anykey = $host.UI.RawUI.ReadKey("NoEcho,IncludeKeyUp")
}
...
Set-AzureSubscription -SubscriptionName $CentralSubscription -CurrentStorageAccount $CentralStorageAccount
Select-AzureSubscription -SubscriptionName $CentralSubscription
...
$VMExportXML.PersistentVM.RoleName = "sushiSCHED-APP-01"
